...
-define(RESOURCE_TYPE_KAFKA, 'kafka').
...
-resource_type(#{name => ?RESOURCE_TYPE_KAFKA,
    ...
    case emqx_rule_utils:http_connectivity(Url) of
        ...
        {error, Reason} ->
            ?LOG(error, "Initiate Resource ~p failed, ResId: ~p, ~0p",
                [?RESOURCE_TYPE_KAFKA, ResId, Reason]),
...
http_request(Url, Headers, Method, Params) ->
    logger:debug("[Kafka Action] ~s to ~s, headers: ~s, body: ~p", [Method, Url, Headers, Params]),
    case do_http_request(Method, ?JSON_REQ(Url, Headers, jsx:encode(Params)),
                         [{timeout, 5000}], [], 0) of
        {ok, _} -> ok;
        {error, Reason} ->
            logger:error("[Kafka Action] HTTP request error: ~p", [Reason]),
